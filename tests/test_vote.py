...
def test_delete_vote_on_post(authorized_client, test_posts, test_vote): #Even though test vote is not being used, it is still included for the function to be called
...
def test_vote_non_exist(authorized_client, test_posts):
    res = authorized_client.post("/vote/", json = {"post_id": 344, "dir": 1})
    assert res.status_code == 404
